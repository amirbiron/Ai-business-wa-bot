...
| `WHATSAPP_PHONE_NUMBER_ID` | מזהה מספר הטלפון מ-Meta Business Manager | לא (נדרש לוואטסאפ) |
| `WHATSAPP_ACCESS_TOKEN` | Access Token מ-Meta WhatsApp API Setup | לא (נדרש לוואטסאפ) |
| `WHATSAPP_VERIFY_TOKEN` | מחרוזת אימות ל-webhook (בוחרים ערך כלשהו) | לא (נדרש לוואטסאפ) |
| `WHATSAPP_BUSINESS_ACCOUNT_ID` | מזהה חשבון עסקי ב-Meta | לא (נדרש לוואטסאפ) |
| `WHATSAPP_OWNER_PHONE` | מספר טלפון בעל העסק לקבלת התראות בוואטסאפ (פורמט: 972501234567) | לא |

---

## שלב 2.5: הקמת ערוץ וואטסאפ (אופציונלי)

> ערוץ וואטסאפ פועל **במקביל** לטלגרם — אין צורך לבחור אחד מהם.

- [ ] גישה ל-[Meta Business Manager](https://business.facebook.com)
- [ ] יצירת Meta Business App עם WhatsApp product
- [ ] הוספת מספר טלפון עסקי (Test Number זמין בחינם לפיתוח)
- [ ] העתקת Phone Number ID מ-API Setup
- [ ] יצירת Permanent Access Token (System User → Generate Token)
- [ ] הגדרת Webhook URL: `https://<your-domain>/webhook/whatsapp`
- [ ] בחירת שדות ל-webhook: `messages` (חובה)
- [ ] הגדרת Verify Token (ערך חופשי — אותו ערך ב-`.env` ובהגדרות Meta)
- [ ] בדיקת Webhook Verification (Meta שולח GET ובודק תשובה)
- [ ] שליחת הודעת בדיקה מ-Test Number ובדיקת לוגים

### מגבלות חשובות בוואטסאפ
- **כפתורים:** מקסימום 3 כפתורי quick reply בהודעה
- **רשימות:** מקסימום 10 פריטים ברשימה
- **Broadcast:** הודעות יזומות (שידור) דורשות **Message Templates** מאושרים ע"י Meta. יש להגיש תבנית דרך Meta Business Manager ולחכות לאישור.
- **חלון 24 שעות:** תגובות ללקוח חייבות להישלח תוך 24 שעות מההודעה האחרונה שלו. אחרי 24 שעות — רק Message Templates.
- **עיצוב טקסט:** `*bold*`, `_italic_`, `~strikethrough~`. **אין** תמיכה ב-HTML.
...
### 5.3 בדיקות בוט וואטסאפ (אם מוגדר)
- [ ] שליחת הודעת "שלום" — ברכה + תפריט כפתורים
- [ ] לחיצה על כפתור "📋 מחירון" — תשובה מ-RAG
- [ ] שאלה חופשית — תשובה מ-RAG עם ציון מקור
- [ ] לחיצה על כפתור "📅 בקשת תור" — זרימת booking (שירות → תאריך → שעה → אישור)
- [ ] לחיצה על כפתור "👤 נציג" — יצירת בקשת נציג + התראה לבעלים
- [ ] בחירת "📍 מיקום" מרשימה — כתובת ודרכי הגעה
- [ ] שליחת הודעת תמונה/קול — הודעת "רק טקסט נתמך"
- [ ] Rate limit — שליחת הודעות רבות ובדיקת חסימה
- [ ] Live chat — הפעלת live chat מהאדמין, ווידוא שהבוט שותק + ההודעות עוברות

### 5.4 בדיקות פאנל אדמין
